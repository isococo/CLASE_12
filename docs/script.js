...
new Chart(document.getElementById("arancelvacantes"), {
    ...
    options: {
        ...
        plugins: {
            legend: { position: "bottom", labels: { font: { family: "'Georama', sans-serif", size: 11 }, color: "#666", boxWidth: 12 } },
            tooltip: { backgroundColor: "#fff", borderColor: "#ddd", borderWidth: 1, titleColor: "#111", bodyColor: "#555", titleFont: { family: "'Georama', sans-serif", size: 12, weight: "600" }, bodyFont: { family: "'Georama', sans-serif", size: 11 }, padding: 10, callbacks: { title: items => items[0].raw.institucion, label: function (context) { const p = context.raw; return [p.programa, p.tipo + " — " + p.region, "Vacantes: " + formatoNumero.format(p.x), "Arancel: " + formatoPesos.format(p.y)]; } } },
        },
    },
});
